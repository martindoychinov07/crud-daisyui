...
export function InputCommon<T extends object, F extends FieldValues>(props: InputProps<T, F>) {
  const { t } = useI18n();
  const focused = useRef<boolean>(false);
  const { register, getValues, setValue } = props.form;
  const value = getValues(props.name as Path<F>);
  ...
  const registered = register(props.name as Path<F>, registrationOptions);
